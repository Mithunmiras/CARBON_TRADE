...
const LiquidChrome: React.FC<LiquidChromeProps> = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      // Dynamically import to avoid build-time type issues if package is not installed
      // @ts-ignore: dynamic import of optional package without types
      ...
      function resize() {
        ...
        resUniform[2] = ((gl.canvas as HTMLCanvasElement).width) / ((gl.canvas as HTMLCanvasElement).height || 1);
      }
    ...
    return () => {
      ...
      if (containerRef.current) {
        // remove any appended canvas children
  ...
  // Merge pointer-events into style so interactive controls work when enabled
  return (
    <div
      ref={containerRef}
      className="liquidChrome-container"
      {...props}
      style={{ ...(props.style as React.CSSProperties), pointerEvents: (interactive ? 'auto' : 'none') as any }}
    />
  );
};
